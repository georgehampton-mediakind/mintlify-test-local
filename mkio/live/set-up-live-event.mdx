---
title: "Set Up a Live Streaming Event with RTMP or SRT in MK.IO"
sidebarTitle: "Live Event Setup"
description: "Step-by-step guide to creating a live event, connecting an RTMP or SRT source, starting the event, and retrieving the playback URL in MK.IO."
---
...
Live content is recorded to your connected cloud storage as an asset, which you can use for VOD playback after the event ends. For a step-by-step walkthrough of the complete live streaming workflow including OBS setup and viewer delivery, see the live streaming getting-started guide.
...
<Steps>
  ...
  <Step title="Configure output settings">
    - **Archive window length**: How long MK.IO retains the live stream recording, from 1 minute up to 7 days. When you stop the event, the full recording is stored as an asset in your cloud storage. When the archive window exceeds 25 hours, the manifest shows only the most recent 25-hour window for viewers. The full recording is still stored. See [Extended archive window](/mkio/live/extended-archive-window).
  ...
  <Step title="Configure advanced settings (optional)">
    ...
    - **AI pipeline** (encoding only): Enable real-time transcription. Select `Predefined_ACSLiveTranscription` and specify the source audio language code (for example, `en-US`). See the transcription transform reference for source language guidance.
  </Step>
...
Live transcription is available for live encoding events. It generates Web Video Text Tracks (WebVTT) subtitles for HLS output and Timed Text Markup Language (TTML) subtitles for DASH output.
...
After the live event is created, you still need to set up streaming infrastructure before viewers can watch. This requires a streaming endpoint and a streaming locator. See [Publish content for streaming](/mkio/delivery/stream-content).
...
Use the [Live streaming API guide](/api-guides/media/live-streaming) to create and control live events and outputs through the Media API.

```bash
# Create a live event via the API
curl -X PUT "https://app.mk.io/api/v1/projects/$MKIO_PROJECT/media/liveEvents/main-stage" \
...
curl -X POST "https://app.mk.io/api/v1/projects/$MKIO_PROJECT/media/liveEvents/main-stage/start" \
  -H "Authorization: Bearer $MKIO_TOKEN"
```
