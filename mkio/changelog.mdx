---
title: "MK.IO Changelog: Latest Product Updates and Features"
sidebarTitle: "Changelog"
description: "Latest product updates for MK.IO — new features, improvements, and changes across live streaming, VOD, billing, AI features, and more."
---

This page lists all product updates for MK.IO in reverse-chronological order. Updates cover new features, improvements, and changes across live streaming, VOD processing, content delivery, AI workflows, billing, and platform infrastructure.
...
The Live and Templates APIs let you automate these workflows and reuse versioned encoding and composition configurations across broadcasts.

See [Live and Multiview](/mkio/live/live-and-multiview), the Live API, and the Templates API for details.
...
See [Billing & Usage](/mkio/admin/billing-and-usage) for details.
...
Payment methods now support display names in the MK.IO user interface and Management API. Use display names to distinguish multiple payment methods associated with the same sales order. The name appears in payment method lists and details.
...
See [Extend the Live Event Archive Window](/mkio/live/extended-archive-window) for details.
...
MK.IO now handles high-volume flow workloads more efficiently, reducing platform load and improving resilience for customers running large-scale workflows.
...
MK.IO now supports on-demand audio dubbing for Video on Demand (VOD) content. Use an AI workflow to generate translated audio tracks from an existing audio track. VOD audio dubbing is available in all regions and is billed per minute of processed content.

See [Multi-language VOD Dubbing](/mkio/vod/ai-dubbing) for details.
...
See the track insertion transform reference for details.
...
See [Set Up a Live Streaming Event](/mkio/live/set-up-live-event) for details.
...
See [Billing & Usage](/mkio/admin/billing-and-usage) for the current billing view.
...
Access your Beam device UI remotely and troubleshoot issues with the new remote access capabilities. You can also add, edit, and remove devices directly in the MK.IO UI without using the API. This release added a unified device-status view and a map of registered device locations.
...
See [Manage Edge Devices](/mkio/admin/edge-devices) for details.
...
See [Configure Webhooks](/mkio/admin/webhooks) for details.
...
MK.IO can now be discovered, subscribed to, and deployed directly through the AWS Marketplace. This simplifies procurement for enterprise customers and makes it easier to integrate MK.IO within your existing AWS infrastructure.
...
Easily update transcription language codes (for example, `en-UK` → `en-GB`) directly from the UI or API — no more manual fixes or request errors.
...
- The `publicName` is used exclusively for published endpoint URLs, ensuring that only approved, share-safe identifiers appear in public addresses
- The `publicName` is validated as globally unique and formatted for compatibility in both DNS names and URL paths

See [Streaming Endpoints](/mkio/delivery/streaming-endpoints) for details.
...
Apple Lossless Audio Codec (ALAC) support is now available for encoding transforms.
...
Sorenson Spark support is now available for encoding transforms to ingest Flash content.
...
ADPCM and DPCM support is now available for WAV content in encoding transforms.
...
See [Monitor Live Streams](/mkio/live/monitor-live-stream) and the activate and scrape operational metrics guide for details.
...
See [Users & Teams](/mkio/admin/users-and-teams) for details.

---

## MP3 ingest and cover images

**October 11, 2024**

The encoding transform can now ingest MP3 files. Cover images are also supported.
...
The encoding transform can now ingest:

- Files in WebM format including AV1 video codec
- Files in WMV format with Microsoft MPEG4 video codec
- Files using the H.263 video codec
...
The encoding transform can now ingest files using the MJPEG video codec.

---

## VP8, VP9, Opus, and Vorbis ingest

**August 29, 2024**

The encoding transform can now ingest files in WebM format including VP8 and VP9 video codecs, and Opus and Vorbis audio codecs.
...
MK.IO is now available in the **West US** region.
...
Live events can now use RTMPS as the ingest transport protocol.
...
## MP4 transition, variable frame rate, thumbnails, and JWT
...
See [API Tokens](/mkio/admin/api-tokens) for details.

---

## Asset conversion presets, MPEG-1, and VC-1 ingest

**June 27, 2024**

- The `CopyTopBitrateInterleaved`, `CopyAllBitratesNonInterleaved`, and `CopyAllBitratesInterleaved` presets became available in all regions. They convert live-event recordings or legacy MP4 content to the current MP4 layout.
...
## Aspect ratio and rotation preserved

**May 29, 2024**

Encoding transforms now preserve the source aspect ratio and rotation information.
...
Integrated CDN delivery launched as a pay-as-you-go capability backed by dedicated endpoint capacity. In May 2026, MK.IO moved integrated CDN delivery from Akamai to Fastly.

See [Streaming Endpoints](/mkio/delivery/streaming-endpoints) for current endpoint options.

---

## OpenID Discovery and multiple options per DRM

**May 8, 2024**

- Support for OpenID Discovery is now available for content key policies with token protection.
- It is now possible to use multiple `ContentKeyPolicyOptions` with the same DRM type in a single content key policy.
...
MK.IO added private network links for Azure Storage accounts in all regions. The connection requires configuration in MK.IO and approval by the storage account owner in Azure.
...
The encoding transform can now ingest ISMV and AVI containers.
...
MK.IO is now available in the **Central US** region.

---

## Persistent licenses and external PlayReady server
...
Three Constant Video Quality (CVQ) presets became available for on-demand transforms in all regions: `H264MultipleBitrateSDWithCVQ`, `H264MultipleBitrate720pWithCVQ`, and `H264MultipleBitrate1080pWithCVQ`. CVQ was available in the Advanced and Premium encoding tiers.

---

## Asset deletion policies and private offer
...
- Private offer support has been introduced to the Azure Marketplace offer to cater specifically to high-volume users and address complex use cases.
...
## DASH & HLS playback and players

**December 1, 2023**

DASH & HLS playback is now available with support for multiple players. Choose your preferred experience with AMS Player, Shaka Player, or MKPlayer, providing enhanced flexibility and compatibility for a seamless video playback journey.

---

## First Quality and asset filters by name

**November 17, 2023**

- Support for the **First Quality** setting, allowing you to set the initial playback quality level directly in the manifest.
- Asset filters now include name-based track filtering.

---

## ClearKey for DASH, latency improvements, security, and new regions

**November 3, 2023**

- ClearKey encryption support for DASH-packed content is now available.
- Enhanced end-to-end latency in live encoding, achieving a 25-second threshold.
- Advanced storage security measures with the latest advancements in encryption, access controls, and threat mitigation.
- Two new availability regions: **Japan West** and **Europe West**.
...
Expanded DRM support for client JWT entitlement management, providing improved control, heightened security, and smoother access management for your content.
...
## Content protection, encoding, and live encoding

**September 6, 2023**

- **Content Key Policies** enable you to control access to content delivered via MK.IO. MK.IO now supports AES-128, ClearKey, and industry-grade multi-DRM options including Widevine, FairPlay, and PlayReady. Access control can be managed by authorizing users based on claims in JWT tokens.
...
- **Live Events** functionality now includes support for live encoding, enabling you to transcode your live event into 720p or 1080p multi-bitrate HLS or DASH output.

---

## Multi-bitrate RTMP, live streaming, and bug fixes
...
- Fixed a bug that resulted in streaming endpoints not starting due to malformed labels.
