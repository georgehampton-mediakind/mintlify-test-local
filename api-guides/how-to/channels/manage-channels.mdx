...
description: Create a channel on a Beam device with a single PUT request, then start, rename, and delete it through the Channels API.
sidebarTitle: Create and manage channels
---
...
Read [How Beam channels work](/api-guides/how-to/channels/how-channels-work) first if you have not picked a `spec.type` yet, because that choice constrains everything else in the body.
...
Entries under `sdi` also carry a `caps` object describing what the port supports, which the older device above does not report. See the `SdiSlotPortInterface` schema in the [Channels API reference](/api-reference/channels-api).
...
Unicast and multicast addresses are both supported, and the Channels API has no field to choose between them: an address in the class D range, `224.0.0.0/4`, is treated as multicast, and anything outside that range is treated as unicast. Where an exception to that check is needed, the Advanced interface offers an explicit [**Unicast**](/beam/receiver/parameters#ip) control.

For the full list of input and output schemas, including SMPTE 2110 and satellite demodulator inputs, see the [Channels API reference](/api-reference/channels-api).
...
* [Monitor channels and devices](/api-guides/how-to/channels/monitor-channels) covers reading status, alarms, and thumbnails once a channel exists.
* [Beam Essentials UI](/beam/essentials) walks the same configuration through the channel creation wizard, which is a useful way to see a valid combination before you script it.
* [Channels API reference](/api-reference/channels-api) holds the complete `ChannelSpec` schema, including every input and output variant.
