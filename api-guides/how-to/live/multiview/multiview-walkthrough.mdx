---
title: Multiview setup walkthrough
description: A complete step-by-step guide to setting up a multiview composition from scratch, from sources and templates to a running Live API event.
...
* A project name and [API token](/api-guides/getting-started/authentication) with access to Media, Templates, and Live resources.
* [Registered storage](/api-guides/how-to/media/storage) for the recording.
...
`networkName` and most transport settings are configured at creation. The source PATCH endpoint supports content reassignment and selected transport changes. See [Update a source](/api-guides/how-to/media/content-and-sources#update-a-source) before recreating a source.
...
The `storageAccountName` must reference a registered storage instance in your project. See [Storage](/api-guides/how-to/media/storage) if you have not registered storage yet.
...
See [Monitor live resources](/api-guides/how-to/live/monitor-live) for the full breakdown of what each status field means.
...
This is live-to-VOD playback of a sliding DVR window while the event is still `Running`, bounded by the `archiveWindowLength` you set in Step 5. See [Streaming and publishing](/api-guides/how-to/media/publishing) for DASH, DRM, and CDN options.
