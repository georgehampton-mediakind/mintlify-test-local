---
title: Monitor live resources
description: Check resource state, read metrics, and access thumbnail and metadata monitoring URLs from the Live API.
...
Each entry in `status.inputs` reflects the resolved assignment for that input. See [Resolving content](/api-guides/how-to/media/content-and-sources#resolving-content) for how the platform gets from `contentName` to one of these:
...
For project-level metrics export and monitoring storage, see [Monitor MK.IO services](/mkio/how-to/monitor-your-services). Use the export URL returned for your project and retain samples in your monitoring system when you need history.
...
4. [Publish the output asset](/api-guides/how-to/media/publishing) and verify the expected video and audio in a compatible player. For multiview, check every tile.
