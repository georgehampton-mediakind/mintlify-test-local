---
title: "Make Your First MK.IO API Call and Verify Token Access"
sidebarTitle: "First API Call"
description: "Verify your API token and project access by making your first authenticated request to the MK.IO API and interpreting the response."
---
...
If you still need a token, follow [Authentication and tokens](/api-guides/getting-started/authentication).
...
Many MK.IO list endpoints use this same shape, so it is worth recognising now. See [Pagination and filtering](/api-guides/concepts/pagination) for how to page and narrow these responses.
...
The error body carries `error.code` for programmatic handling, `error.detail` for logs, and `ref` for support follow-up. See [Error handling](/api-guides/concepts/error-handling) for the full model.
...
<CardGroup cols={3}>
  <Card title="Which API to Use" icon="map" href="/api-guides/getting-started/which-api">
    Choose the API that owns your workflow.
  </Card>
  <Card title="API Overview" icon="book" href="/api-guides/concepts/overview">
    The patterns shared across the APIs.
  </Card>
  <Card title="Build with the Media API" icon="film" href="/api-guides/media/overview">
    Move into storage, assets, transforms, jobs, and publishing.
  </Card>
</CardGroup>
