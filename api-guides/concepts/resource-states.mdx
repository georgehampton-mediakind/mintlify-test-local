---
title: "MK.IO API Resource States and Lifecycle"
sidebarTitle: "Resource States"
description: "Track MK.IO resource lifecycle states — how assets, jobs, live events, and endpoints move through states and how to monitor state via polling or webhooks."
---
...
<Note>
  Live API channels, events, and multiview resources also expose `/state`. Their desired state is `spec.state`, and their reported state is `status.state`. See [Live resource states](/api-guides/live/core-concepts) for that lifecycle. The Media API examples below use a different resource model.
</Note>
...
Each resource moves through its own lifecycle. The table below lists the states you observe in normal operation. Use them to decide when the next step in a workflow is valid.
...
<Note>
  The API may also report extra transient states for some resources, such as `Updating`, `Scaling`, or `Error`. The [Media API reference](/api-reference/media-api) lists the complete enum for each resource, while the [resource states reference](/api-guides/concepts/resource-states) explains what each value means.
</Note>
...
<Steps>
  <Step title="Create or start the resource">
    Issue the create or start request and receive the initial response.
  </Step>
  <Step title="Poll until ready">
    Poll until it reaches the state that makes the next step valid.
  </Step>
  <Step title="Make the next call">
    Make the next call only after the resource has reached the expected state.
  </Step>
</Steps>
...
- For Media API `/media/liveEvents`, follow the [Media live-streaming startup sequence](/api-guides/media/live-streaming).
- For Live API `/live/liveEvents`, prepare the contribution encoder as part of startup and monitor `status.state`. See the [Live event walkthrough](/api-guides/live/manage-live#step-6-start-the-event).
- Wait for a streaming endpoint to reach `Running` before you expect playback to succeed.

## When to stop polling

Polling suits short waits and one-off scripts. It becomes expensive when you monitor many jobs or long-running live workflows, because each check is a request that counts against the [rate limits](/api-guides/concepts/rate-limits). At that scale, [webhooks](/api-guides/concepts/webhooks) deliver the same state changes without the repeated reads. A common pattern is to use webhooks for change notification and a single `GET` for current detail when a user asks for it.

## What comes next

<CardGroup cols={2}>
  <Card title="Webhooks" icon="bell" href="/api-guides/concepts/webhooks">
    Replace repeated polling with event delivery.
  </Card>
  <Card title="Transforms and Jobs" icon="gear" href="/api-guides/media/transforms-and-jobs">
    See how job state fits a VOD workflow.
  </Card>
  <Card title="Live Streaming" icon="video" href="/api-guides/media/live-streaming">
    See how live-event state affects ingest and publishing.
  </Card>
  <Card title="Rate Limits" icon="gauge" href="/api-guides/concepts/rate-limits">
    Understand the cost of polling and how to reduce it.
  </Card>
</CardGroup>
