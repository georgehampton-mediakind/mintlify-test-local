---
title: "Receive MK.IO Event Notifications with Webhook Rules"
sidebarTitle: "Webhooks"
description: "Subscribe to MK.IO API events with webhook rules to receive HTTP callbacks when jobs complete, live events change state, or other resource events occur."
---
...
The configuration endpoint is `/api/v1/projects/{project_name}/webhook/rules`. For the create, edit, and delete calls, see [Webhook rules](/api-guides/management/webhooks).
...
<Steps>
  <Step title="Create the resource">
    Create the resource as usual.
  </Step>
  <Step title="Return control">
    Return control to the user or job runner immediately.
  </Step>
  <Step title="Wait for the event">
    Wait for the event that reports the state change.
  </Step>
  <Step title="Retrieve fresh detail when needed">
    Retrieve the resource only when you need fresh detail.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Resource States" icon="circle-dot" href="/api-guides/concepts/resource-states">
    The state values these events report.
  </Card>
  <Card title="Rate Limits" icon="gauge" href="/api-guides/concepts/rate-limits">
    Why event delivery scales better than polling.
  </Card>
</CardGroup>
