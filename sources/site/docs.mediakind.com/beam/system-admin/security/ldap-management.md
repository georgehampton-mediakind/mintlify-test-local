# Source: https://docs.mediakind.com/beam/system-admin/security/ldap-management

# LDAP management

## LDAP configuration

[Section titled “LDAP configuration”](https://docs.mediakind.com/beam/system-admin/security/ldap-management/#ldap-configuration)
...
4. In the **LDAP** tab, configure the LDAP settings to integrate with the local LDAP server.
 ...
 ![menu system settings ldap](https://docs.mediakind.com/_astro/menu_system_settings_ldap.Zjyr7mfQ_Z18FKME.webp)
...
[Section titled “Loading LDAP certificates”](https://docs.mediakind.com/beam/system-admin/security/ldap-management/#loading-ldap-certificates)
...
3. Edit the config file to set the right values:
 ...
 | `LDAP group mappings` | A section that can be used to control authorisation through LDAP groups. Configure `group_search` (and don’t use `default group`). In the example, any user that belongs to the LDAP group `MK_Configuration` will be given the privileges for the Controller group `Configuration`. | `LDAP group` |
 | `MK_Admin` | `Admin` | |
