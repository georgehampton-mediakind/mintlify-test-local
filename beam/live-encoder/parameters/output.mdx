---
title: Output configuration parameters
sidebarTitle: Output
---
...
**Note:** See [MPEG-2 TS common parameters](/beam/live-encoder/parameters/output#mpeg-2-ts-common-parameters) for specific parameters.
...
| Parameter                  | Description                                                                                                                                                                                                                                           |
| :------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Publishing point           | The URL of the Packager entry point, to which the Live Encoder will send the stream (e.g., [https://example.com/ingest1](https://example.com/ingest1)).                                                                                               |
| Secondary publishing point | The URL of the secondary Packager entry point, to which the Live Encoder will send a duplicate stream (e.g., [https://example.com/ingest2](https://example.com/ingest2)). You can configure up to 5 publishing points by clicking the **Add** button. |

**Note:** See [DASH-IF CMAF parameters](/beam/live-encoder/parameters/output#dash-if-cmaf-parameters) for specific parameters.

**Related information**\
[Configuring the output stream general parameters](/beam/live-encoder/configure/output/configure-output#configure-the-output-stream-general-parameters)
...
| Signal loss management                | Defines the behavior in case of signal loss.None: this corresponds to the default behavior. The encoder follows the freeze frame management settings. The output will contain either a color bar, a black frame, the last image (by default) or a slate.  Drop video stream: if the input video elementary stream is lost, the corresponding packets are dropped and replaced by stuffing.  Drop transport stream: this option allows dropping the whole transport stream. The Encoder also mutes the statmux communication. |
...
| Direct Path for low latency           | Activates the optimized data transmission between MK Live Encoding and MK Packaging for OTT latency. It is required to activate this mode on both encoding and packaging sides.                                                                                                                                                                                                                                                                                                                                              |
...
**Related information**\
[Configure the output stream MPEG-2 TS parameters](/beam/live-encoder/configure/output/configure-output#configure-the-output-stream-mpeg-2-ts-parameters)
...
Click **Add** to include tracks that will be added to the output. See [Define the output stream composition.](/beam/live-encoder/configure/output/configure-output#define-the-output-stream-composition)

**Related information**\
[Configure a DASH-IF CMAF output](/beam/live-encoder/configure/output/configure-output#configure-a-dash-if-cmaf-output)

## Output stream parameters

<span id="mpeg-2-ts-sdi-sdiip-or-smpte-st-2110-parameters" className="mk-legacy-anchor" />
...
| FEC          | Activate FEC (Forward Error Correction) along with the output transport stream to resolve RTP packet loss issues due to network transport. Only available in case of MPEG2 TS/RTP. |
